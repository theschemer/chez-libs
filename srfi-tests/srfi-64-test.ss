  ...
  (let ((r (test-runner-null))
    ...
    (test-runner-on-bad-count!
     ...
     (lambda (runner count expected-count)
       (error 'test-runner-on-bad-count!
              (string-append "bad count " (number->string count)
                             " but expected "
                             (number->string expected-count)))))
    (test-runner-on-bad-end-name!
      r
      (lambda (runner begin end)
        (error 'test-runner-on-bad-end-name!
               (string-append "bad end group name " end
                              " but expected " begin))))
    (test-runner-on-test-end! 
      r 
      (lambda (runner)
