;; Test suite taken from chibi scheme 

(import (except (rnrs) fold-right for-each map remove)
        (srfi :1 lists)
        (srfi :64 testing))

(test-begin "srfi-1-lists")
...
(test-assert (memp (lambda (x) (equal? x (take! (circular-list 1 3 5) 8))) '((1 3) (1 3 5 1 3 5 1 3))))
...
(test-assert (memp (lambda (x) (equal? x (let ((count 0)) (map (lambda (ignored) (set! count (+ count 1)) count) '(a b))))) '((1 2) (2 1))))
