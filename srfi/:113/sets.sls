(library 
  ...
  (define-record-type (sob raw-make-sob sob?)
                      (fields
                        (immutable hash-table sob-hash-table)
                        (immutable comparator sob-comparator)
                        (immutable multi? sob-multi?)))

  (define (set? obj) (and (sob? obj) (not (sob-multi? obj))))

  (define (bag? obj) (and (sob? obj) (sob-multi? obj)))

  (define (check-set obj) (if (not (set? obj)) (error 'check-set "not a set" obj)))

  (define (check-bag obj) (if (not (bag? obj)) (error 'check-bag "not a bag" obj)))
  ...
  (define (check-same-comparator a b)
    (if (not (eq? (sob-comparator a) (sob-comparator b)))
      (error 'check-same-comparator "different comparators" a b)))
