(library 
  ...
  (export iq
          ...
          iapply iequal?)
  (import (rnrs) (rnrs mutable-pairs) (only (srfi :1 lists) every) (srfi :8 receive))
  ...
  (define-record-type (<ilist> ipair ipair?) (fields (immutable icar icar) (immutable icdr icdr)))
  ...
  (define (check-arg pred val caller)
    (if (pred val) val (error 'check-arg "Bad argument" val pred caller)))
    ...
    (syntax-rules ()
                  ((:optional rest default)
                   (cond
                     ...
                     (else (error ':optional "Too many arguments"))))))
  ...
  (define (make-ilist len . maybe-elt)
    ...
    (let ((elt (cond ((null? maybe-elt) #f) ; Default value
                     ...
                     (else (error 'make-ilist "Too many arguments to MAKE-ILIST"
                                  (ipair len maybe-elt))))))
      (do ((i len (- i 1))
           (ans '() (ipair elt ans)))
        ((<= i 0) ans))))
  ...
  (define (iiota count . maybe-start+step)
    ...
    (if (< count 0) (error 'iiota "Negative step count" iiota count))
  ...
  (define (null-ilist? l)
    (cond ((ipair? l) #f)
          ((null? l) #t)
          (else (error 'null-ilist? "null-ilist?: argument out of domain" l))))
