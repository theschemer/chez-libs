;; Srfi 19  Time Date Types and Procedures

;; -- Chez Scheme supports these operations directly
...
(library
  ...
  ;; it also calls the almost standard, but not R5 procedures get-line
  ;; & open-input-string
    ...
    (let ( (port (open-input-file filename))
      ...
      (let loop ((line (get-line port)))
        (if (not (eof-object? line))
          (begin
            (let* ( (data (read (open-string-input-port (string-append "(" line ")"))))
                   (year (car data))
                   (jd   (cadddr (cdr data)))
  ...
  (define (tm:get-time-of-day)
    (let ((time (chez:current-time)))
      (values (chez:time-second time)
              (* 1000 (chez:time-nanosecond time)))))
...
;    (tm:current-time-ms-time time-gc current-gc-milliseconds))
  ...
  (define-record-type (date make-date date?)
                      (fields
                        (mutable nanosecond date-nanosecond set-date-nanosecond!)
                        (mutable second date-second set-date-second!)
                        (mutable minute date-minute set-date-minute!)
                        (mutable hour date-hour set-date-hour!)
                        (mutable day date-day set-date-day!)
                        (mutable month date-month set-date-month!)
                        (mutable year date-year set-date-year!)
                        (mutable zone-offset date-zone-offset set-date-zone-offset!)))
...
;    (tm:time-error 'set-date-zone-offset! 'dates-are-immutable date))
  ...
  (define (tm:local-tz-offset)
    (chez:date-zone-offset (chez:current-date)))
;    (date-time-zone-offset (seconds->date (current-seconds))))
  ...
  (define (tm:time->date time tz-offset ttype)
    ...
    (let* ( (offset (if (null? tz-offset) (tm:local-tz-offset) (car tz-offset)) ))
  ...
  (define current-date 
    (case-lambda 
      (()
       (current-date (tm:local-tz-offset)))
      ((tz-offset)
  ...
  (define julian-day->date
    (case-lambda
      ((jdn)
       (julian-day->date jdn (tm:local-tz-offset)))
      ((jdn offset)
  ...
  (define modified-julian-day->date
    (case-lambda
      ((jdn)
       (modified-julian-day->date jdn (tm:local-tz-offset)))
      ((jdn offset)
  ...
  (define date->string
    (case-lambda 
      ((date)
       (date->string date "~c"))
      ((date fmt-str)
       (let-values (((str-port get-output-string) (open-string-output-port)))
  ...
  (define (tm:locale-reader port indexer)
    (let-values (((string-port get-output-string) (open-string-output-port)))
