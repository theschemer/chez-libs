...
(library
  ...
  (import (except (rnrs base) for-each map) 
          (rnrs control) 
          (except (rnrs lists) fold-right remove) 
          (rnrs mutable-pairs)
          (srfi :8 receive))
  ...
  (define (zip list1 . more-lists) (apply map-in-order list list1 more-lists))
  ...
  ;; for-each which ends cleanly if lists of unequal length, as in R7RS
  ...
  ;;; find-tail take-while drop-while span break any every list-index
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (define (%lset2<= = lis1 lis2) (every (lambda (x) (memp (lambda (y) (= x y)) lis2)) lis1))
  ...
  (define (lset-adjoin = lis . elts)
    ...
    (fold (lambda (elt ans) (if (memp (lambda (x) (= x elt)) ans) ans (cons elt ans)))
          lis elts))
    ...
    (let ((lists (delete lis1 lists eq?))) ; Throw out any LIS1 vals.
      (cond ((any null-list? lists) '())		; Short cut
            ...
            (else (filter (lambda (x)
                            (every (lambda (lis) (memp (lambda (y) (= x y)) lis)) lists))
                          lis1)))))
  ...
  (define (lset-difference = lis1 . lists)
    ...
    (let ((lists (filter pair? lists)))	; Throw out empty lists.
      (cond ((null? lists)     lis1)	; Short cut
            ...
            (else (filter (lambda (x)
                            (every (lambda (lis) (not (memp (lambda (y) (= x y)) lis)))
                                   lists))
                          lis1)))))
              ...
              (receive (a-b a-int-b)   (lset-diff+intersection = a b)
                       (cond ((null? a-b)     (lset-difference = b a))
                             ...
                             (else (fold (lambda (xb ans)
                                           (if (memp (lambda (xx) (= xx xb)) a-int-b) ans (cons xb ans)))
                                         a-b
                                         b)))))
            '() lists))
              ...
              (receive (a-b a-int-b)   (lset-diff+intersection! = a b)
                       (cond ((null? a-b)     (lset-difference! = b a))
                             ...
                             (else (pair-fold (lambda (b-pair ans)
                                                (if (memp (lambda (x) (= x (car b-pair))) a-int-b) ans
  ...
  (define (lset-diff+intersection = lis1 . lists)
    ...
    (cond ((every null-list? lists) (values lis1 '()))	; Short cut
          ...
          (else (partition (lambda (elt)
                             (not (any (lambda (lis) (memp (lambda (x) (= x elt)) lis))
                                       lists)))
                           lis1))))
