...
;; 
;; Implementation of slib/array.scm
;;
...
;; Repackaged for R6RS Scheme by Peter Lane, 2017
...
(library
  ...
  (define-record-type (<array> new-array srfi-array?)
                      (fields
                        (immutable dimensions get-dimensions )
                        (immutable scales get-scales )     ; list of dimension scales
                        (immutable offset get-offset )     ; exact integer
                        (immutable store get-store )))     ; data

  (define (array:dimensions obj)
    (cond ((vector? obj) (list (vector-length obj)))
          ((string? obj) (list (string-length obj)))
          (else (get-dimensions obj))))
            ...
            (else
              (let ((store
                ...
                (define (loop dims scales)
                  (if (null? dims)
                    (new-array dimensions (cdr scales) 0 store)
                    (loop (cdr dims)
