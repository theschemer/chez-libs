...
(library 
  ...
  (import (except (rnrs) vector->list vector-fill!)
          (srfi :8 receive))
    ...
    (define-syntax let*-optionals:aux
      (syntax-rules ()
                    ((aux ?orig-args-var ?args-var () ?body1 ?body2 ...)
                     (if (null? ?args-var)
                       (let () ?body1 ?body2 ...)
                       (error 'let*-optionals:aux
                              "too many arguments" (length ?orig-args-var)
                              ?orig-args-var)))
    ...
    (define (check-type pred? value callee)
      (if (pred? value)
        value
        (error 'check-type "Check-type: value does not satisfy predicate")))
    ...
    (define (check-index vec index callee)
      (let ((index (check-type integer? index callee)))
        (cond ((< index 0)
               (error 'check-index "check-index: vector index too low"))
              ((>= index (vector-length vec))
               (error 'check-index "check-index: vector index too high"))
              (else index))))
    ...
    (define vector-copy
      (case-lambda
        ...
        ((vec start end)
         (let ((new-vector (make-vector (- end start))))
        ...
        (cond ((null? args)
              ...
              ((null? (cddr args))
               (check-indices vec
                              ...
                              callee))
              (else
                (error 'vector-parse-start+end "too many arguments"
    ...
    (define vector-fill!
      (case-lambda 
        ((vec val)
         (vector-fill! vec val 0 (vector-length vec)))
        ((vec val start)
    ...
    (define vector->string
      (case-lambda
        ((vec)
         (vector->string vec 0 (vector-length vec)))
        ((vec start)
         (vector->string vec start (vector-length vec)))
        ((vec start end)
         (do ((i (- end 1) (- i 1))
              (result '() (cons (vector-ref vec i) result)))
           ((< i start) (apply string result))))))

    (define string->vector
      (case-lambda
        ((str)
         (string->vector str 0 (string-length str)))
        ((str start)
         (string->vector str start (string-length str)))
        ((str start end)
         (let* ((size (- end start))
                (res (make-vector (- end start))))
           (do ((i 0 (+ 1 i)))
             ((>= i size) res)
             (vector-set! res i (string-ref str (+ start i))))))))
