...
(library 
  ...
  (import (chezscheme)) ;; requires make-parameter from Chez scheme
                  ...
                  ((:range cc var arg1 arg2 arg3)
                   (:do cc
                        (let ((a arg1) (b arg2) (s arg3) (stop 0))
                          ...
                          (if (zero? s)
                            (error 'range "step size must not be zero in :range") )
                   ...
                   (:do cc
                        (let ((a arg1) (b arg2) (s arg3) (istop 0))
                          (if (not (and (real? a) (real? b) (real? s)))
                            (error 'real-range "arguments of :real-range are not real" a b s) )
                  ...
                  ((:dispatched cc var dispatch arg1 arg ...)
                   (:do cc
                        (let ((d dispatch) 
                          ...
                          (set! g (d args))
                          (if (not (procedure? g))
                            (error 'dispatched "unrecognized arguments in dispatching" 
                                   args 
                                   (d '()) )))
                        ((var (g empty)))
                        (not (eq? var empty))
  ...
  (define-syntax :generator-proc
    (syntax-rules (:do let)
                  ...
                  ((:generator-proc var)
                   (error 'generator-proc "illegal macro call") )))
        ...
        (if g1
          (if g2 
            (if (null? args)
              ...
              (error 'dispatch-union "dispatching conflict" args (d1 '()) (d2 '())) )
            g1 )
          (if g2 g2 #f) ))))
                       ...
                       (do-ec qualifier
                              (if (< i len)
                                (begin (vector-set! vec i expression)
                                       (set! i (+ i 1)) )
                                (error 'vector-of "vector is too short for the comprehension") ))
                       (if (= i len)
                         vec
                         (error 'vector-of "vector is too long for the comprehension") ))))))
  ...
  (define-syntax min-ec
    (syntax-rules ()
                  ((min-ec etc1 etc ...)
                   (fold3-ec (mymin) etc1 etc ... min min) )))
  ...
  (define-syntax max-ec
    (syntax-rules ()
                  ((max-ec etc1 etc ...)
                   (fold3-ec (mymax) etc1 etc ... max max) )))
  ...
  (define-syntax every?-ec
    ...
    (define-syntax (index i)
      (error 'syntax #f "misplaced aux keyword index"))
    (define-syntax (nested i)
      (error 'syntax #f "misplaced aux keyword nested"))
